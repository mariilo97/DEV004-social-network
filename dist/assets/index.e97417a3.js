...
function validateIndexedDBOpenable() {
  return new Promise((resolve, reject2) => {
    try {
      ...
      request.onerror = () => {
        var _a;
        reject2(((_a = request.error) === null || _a === void 0 ? void 0 : _a.message) || "");
      };
    } catch (error) {
      reject2(error);
    }
  ...
  constructor(auth) {
    ...
    this.promise = new Promise((_, reject2) => {
      this.timer = setTimeout(() => {
        return reject2(_createError(this.auth, "network-request-failed"));
...
class RecaptchaEnterpriseVerifier {
  ...
  async verify(action = "verify", forceRefresh = false) {
    async function retrieveSiteKey(auth) {
      ...
      return new Promise(async (resolve, reject2) => {
        getRecaptchaConfig(auth, {
          ...
          if (response.recaptchaKey === void 0) {
            reject2(new Error("recaptcha Enterprise site key undefined"));
          } else {
    ...
    function retrieveRecaptchaToken(siteKey, resolve, reject2) {
      ...
      if (isEnterprise(grecaptcha)) {
        ...
        reject2(Error("No reCAPTCHA enterprise script loaded."));
      }
    }
    return new Promise((resolve, reject2) => {
      retrieveSiteKey(this.auth).then((siteKey) => {
        if (!forceRefresh && isEnterprise(window.grecaptcha)) {
          retrieveRecaptchaToken(siteKey, resolve, reject2);
        } else {
          if (typeof window === "undefined") {
            reject2(new Error("RecaptchaVerifier is only supported in browser"));
            return;
          }
...
class AuthMiddlewareQueue {
  ...
  pushCallback(callback, onAbort) {
    const wrappedCallback = (user2) => new Promise((resolve, reject2) => {
      try {
...
function initializeAuth(app2, deps) {
  ...
  if (provider2.isInitialized()) {
    const auth2 = provider2.getImmediate();
  ...
  const auth = provider2.initialize({ options: deps });
  return auth;
}
...
class MultiFactorError extends FirebaseError {
  constructor(auth, error, operationType, user2) {
    var _a;
  ...
  static _fromErrorAndOperation(auth, error, operationType, user2) {
    return new MultiFactorError(auth, error, operationType, user2);
  }
}
function _processCredentialSavingMfaContextIfNecessary(auth, operationType, credential, user2) {
  ...
  return idTokenProvider.catch((error) => {
    if (error.code === `auth/${"multi-factor-auth-required"}`) {
      throw MultiFactorError._fromErrorAndOperation(auth, error, operationType, user2);
    }
    throw error;
  });
}
  ...
  async _send(eventType, data, timeout = 50) {
    ...
    return new Promise((resolve, reject2) => {
      ...
      const ackTimer = setTimeout(() => {
        reject2(new Error("unsupported_event"));
      }, timeout);
      handler = {
        messageChannel,
        onMessage(event) {
          ...
          switch (messageEvent.data.status) {
            ...
            default:
              clearTimeout(ackTimer);
              clearTimeout(completionTimer);
              reject2(new Error("invalid_response"));
              break;
          }
        }
      };
...
class AbstractPopupRedirectOperation {
  constructor(auth, filter2, resolver, user2, bypassAuthState = false) {
    this.auth = auth;
  ...
  execute() {
    return new Promise(async (resolve, reject2) => {
      this.pendingPromise = { resolve, reject: reject2 };
      try {
...
async function signInWithPopup(auth, provider2, resolver) {
  const authInternal = _castAuth(auth);
  _assertInstanceOf(auth, provider2, FederatedAuthProvider);
  const resolverInternal = _withDefaultResolver(authInternal, resolver);
  const action = new PopupOperation(authInternal, "signInViaPopup", provider2, resolverInternal);
  return action.executeNotNull();
}
class PopupOperation extends AbstractPopupRedirectOperation {
  constructor(auth, filter2, provider2, resolver, user2) {
    super(auth, filter2, resolver, user2);
  ...
  return new Promise((resolve, reject2) => {
    ...
    function loadGapiIframe() {
      ...
      gapi.load("gapi.iframes", {
        ...
        ontimeout: () => {
          resetUnloadedGapiModules();
          reject2(_createError(auth, "network-request-failed"));
        },
    ...
    if ((_b = (_a = _window().gapi) === null || _a === void 0 ? void 0 : _a.iframes) === null || _b === void 0 ? void 0 : _b.Iframe) {
      ...
      return _loadJS(`https://apis.google.com/js/api.js?onload=${cbName}`).catch((e) => reject2(e));
    }
  }).catch((error) => {
...
async function _openIframe(auth) {
  ...
  return context.open({
    ...
    dontclear: true
  }, (iframe) => new Promise(async (resolve, reject2) => {
    await iframe.restyle({
      setHideOnLeave: false
    });
    const networkError = _createError(auth, "network-request-failed");
    const networkErrorTimer = _window().setTimeout(() => {
      reject2(networkError);
    }, PING_TIMEOUT.get());
...
async function _getRedirectUrl(auth, provider2, authType, redirectUrl, eventId, additionalParams) {
  ...
  if (provider2 instanceof FederatedAuthProvider) {
    provider2.setDefaultLanguage(auth.languageCode);
...
class BrowserPopupRedirectResolver {
  ...
  async _openPopup(auth, provider2, authType, eventId) {
    var _a;
    debugAssert((_a = this.eventManagers[auth._key()]) === null || _a === void 0 ? void 0 : _a.manager, "_initialize() not called before _openPopup()");
    const url = await _getRedirectUrl(auth, provider2, authType, _getCurrentUrl(), eventId);
    return _open(auth, url, _generateEventId());
  }
  async _openRedirect(auth, provider2, authType, eventId) {
...
const firebaseConfig = {
  apiKey: "AIzaSyAQV2yYkkqZ130HzaqeTEaXjtKoWbv6BQM",
  authDomain: "mama-genial-ml.firebaseapp.com",
  projectId: "mama-genial-ml",
  storageBucket: "mama-genial-ml.appspot.com",
  messagingSenderId: "108230837847",
  appId: "1:108230837847:web:6371c4dc1b5698ee2d4512"
};
...
const createUser = (name2, email, password) => {
...
const authStateChangedevent = (cb2) => {
  const auth = getAuth();
  onAuthStateChanged(auth, (user2) => cb2(user2));
};
...
const refPost = () => {
  return Rl(_h(db, "post"), Cl("dateCreate", "desc"));
};
...
const Home = () => {
  ...
  h1.textContent = "Para Continuar, Inicia Sesi\xF3n";
  buttonLoginGoogle.addEventListener("click", () => {
    loginGoogle().then((credential) => {
      const user2 = credential.user;
      console.log(user2);
      if (user2 !== void 0)
        onNavigate("/feed");
    }).catch((error) => {
  ...
  buttonLogin.addEventListener("click", () => {
    if (inputEmail.value === "" || inputPassword.value === "") {
      swal({
        title: "\xA1Verifica tus datos!",
        text: "la contrase\xF1a debe ser mayor a 6 digitos",
        icon: "info",
        dangerMode: true
      });
    } else {
      console.log(inputEmail.value, inputPassword.value);
      loginUser(inputEmail.value, inputPassword.value).then((res) => {
        console.log(res);
        if (user !== void 0) {
          onNavigate("/feed");
        }
        ...
        swal({
          title: "\xA1Usuario no registrado!",
...
const Register = () => {
  ...
  buttonRegister.addEventListener("click", () => {
    if (inputEmail.value === "" || inputPassword.value === "") {
      swal({
        title: "\xA1Completa todos los campos!",
        text: "la contrase\xF1a debe ser mayor a 6 digitos",
        icon: "info",
        dangerMode: true
      });
    } else {
      console.log(inputName.value, inputEmail.value, inputPassword.value);
      createUser(inputName.value, inputEmail.value, inputPassword.value).then((res) => {
        console.log(res);
...
function reduce(coll, memo, iteratee, callback) {
  callback = once(callback);
  var _iteratee = wrapAsync(iteratee);
  return eachOfSeries$1(coll, (x2, i, iterCb) => {
    _iteratee(memo, x2, (err, v2) => {
      memo = v2;
      iterCb(err);
    });
...
function _createTester(check, getResult) {
  return (eachfn, arr, _iteratee, cb2) => {
    ...
    eachfn(arr, (value, _, callback) => {
      ...
      if (err)
        return cb2(err);
      cb2(null, testPassed ? testResult : getResult(false));
    });
  };
}
...
function filterGeneric(eachfn, coll, iteratee, callback) {
  ...
  eachfn(coll, (x2, index, iterCb) => {
    ...
    if (err)
      return callback(err);
    callback(null, results.sort((a, b2) => a.index - b2.index).map((v2) => v2.value));
  });
}
...
function reject(eachfn, arr, _iteratee, callback) {
  const iteratee = wrapAsync(_iteratee);
  return _filter(eachfn, arr, (value, cb2) => {
    iteratee(value, (err, v2) => {
      cb2(err, !v2);
    });
  }, callback);
}
...
function sortBy(coll, iteratee, callback) {
  ...
  return map$1(coll, (x2, iterCb) => {
    ...
    if (err)
      return callback(err);
    callback(null, results.sort(comparator).map((v2) => v2.value));
  });
...
const Feed = () => {
  ...
  inputFeed.placeholder = "\xBFC\xF3mo te sientes hoy?";
  ...
  pf(refPost(), (querySnapshot) => {
    ...
    querySnapshot.forEach((post) => {
      ...
      articlePost.append(strong, p2, inputEditable, botonesPost);
